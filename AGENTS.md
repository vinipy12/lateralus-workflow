...
## Slice Handoff
When a completed slice is large enough to justify a pull request, stop expanding the implementation, push the branch, and open a PR with a grounded title and description based on the actual diff and verification. After the PR is open, request `@codex review` when review is enabled, then babysit the PR: watch for Codex review comments, decide whether each comment is relevant, fix relevant comments narrowly, commit and push the fixes, reply to the review thread, and request `@codex review` again. Keep iterating until Codex says the branch looks good or no major issues were found. If Codex review stalls, fails, or asks for a product decision, report the exact blocker instead of starting unrelated work.
