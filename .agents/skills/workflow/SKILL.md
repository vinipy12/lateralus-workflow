...
2. Keep the phase boundary explicit:
   ...
   - Record review outcomes inline on `set-step-status ... fix_pending|commit_pending` with `--review-summary`, `--scope-confirmed true|false`, repeatable `--scope-reviewed-path` for reviewed step-owned paths, `--verification-status passed|blocked`, repeatable `--verification-command` for every command that ran when verification passed, `--verification-note` when blocked, repeatable `--residual-testing-gap` for passing reviews, repeatable `--agents-checked`, `--agents-updated true|false`, `--finding-count <n>`, and one repeatable `--review-finding` JSON object per finding when the review fails.
